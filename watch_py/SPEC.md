...
├── config.py               # Pins, UUIDs, timing constants, colour palette, font loaders
├── settings.json           # Persisted settings on device flash (auto-created)
│
├── fonts/
│   ├── montserrat_42.bin   # Binary font 42px — time display (generated by lv_font_conv)
...
│   ├── wifi.py             # WiFi NTP time sync via BLE-provided credentials
