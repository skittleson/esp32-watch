...
#   - Centre: digital time (12h, Montserrat 28) with seconds (opacity toggles on even/odd)
...
#   - Goal-met arc: innermost ring, lights up full cyan when step goal reached
#
# Performance: 16 widgets total (3 arcs, 8 labels, 1 divider, 4 ticks).
...
class ClockFace:
    ...
    def _build_ui(self):
        ...
        # ── Goal-met arc (innermost ring, hidden until goal reached) ──────
        self._goal_arc = lv.arc(scr)
        self._goal_arc.set_size(180, 180)
        self._goal_arc.center()
        self._goal_arc.set_bg_angles(_ARC_BG_START, _ARC_BG_END)
        self._goal_arc.set_angles(_ARC_BG_START, _ARC_BG_START)
        self._goal_arc.set_style_arc_color(_c(0x001111), lv.PART.MAIN)  # dim cyan track
        self._goal_arc.set_style_arc_width(4, lv.PART.MAIN)
        self._goal_arc.set_style_arc_rounded(True, lv.PART.MAIN)
        self._goal_arc.set_style_arc_color(_c(C_NEON_CYAN), lv.PART.INDICATOR)
        self._goal_arc.set_style_arc_width(4, lv.PART.INDICATOR)
        self._goal_arc.set_style_arc_rounded(True, lv.PART.INDICATOR)
        self._goal_arc.remove_style(None, lv.PART.KNOB)
        self._goal_arc.remove_flag(lv.obj.FLAG.CLICKABLE)
        self._goal_arc.set_style_bg_opa(lv.OPA.TRANSP, 0)

        # ── Time label (28px compiled-in Montserrat, neon cyan) ─────────
        ...
        self._time_lbl.align(lv.ALIGN.CENTER, 0, -26)
        ...
        self._sec_lbl.set_style_text_font(get_font_medium(), 0)
        self._sec_lbl.set_style_text_color(_c(C_NEON_CYAN), 0)
        self._sec_lbl.set_text(":00")
        self._sec_lbl.align(lv.ALIGN.CENTER, 0, 10)

        # ── Neon divider line (centered via bounding box) ─────────────────
        line = lv.line(scr)
        pts = [{"x": 0, "y": 0}, {"x": 140, "y": 0}]
        ...
        line.align(lv.ALIGN.CENTER, 0, 28)
        ...
        self._date_lbl.align(lv.ALIGN.CENTER, 0, 42)
        ...
        # ── Temp label (bottom-left, orange accent, inside arcs) ──────────
        ...
        self._temp_lbl.align(lv.ALIGN.CENTER, -46, 58)

        # ── Step count label (bottom-right, magenta accent, inside arcs) ──
        ...
        self._steps_lbl.align(lv.ALIGN.CENTER, 38, 58)
        ...
        self._prev_step_goal = None
        self._prev_goal_met = None
    ...
    def update(self, shared):
        ...
        if time_str != self._prev_time or ampm != self._prev_ampm:
            ...
            self._ampm_lbl.align_to(self._time_lbl, lv.ALIGN.OUT_RIGHT_TOP, 4, 4)
        ...
        # Steps arc + label + goal-met arc
        steps = shared.get("steps", 0)
        step_goal = shared.get("step_goal", DEFAULT_STEP_GOAL)
        if steps != self._prev_steps or step_goal != self._prev_step_goal:
            self._update_step_arc(steps, step_goal)
            self._prev_steps = steps
            self._prev_step_goal = step_goal
    ...
    def _update_step_arc(self, steps, goal):
        sweep = int(min(steps, goal) * 270 // goal)
        end_angle = (_ARC_BG_START + sweep) % 360
        ...
        # Goal-met arc: full bright ring when goal reached, empty otherwise
        goal_met = steps >= goal
        if goal_met != self._prev_goal_met:
            if goal_met:
                self._goal_arc.set_angles(_ARC_BG_START, _ARC_BG_END)
            else:
                self._goal_arc.set_angles(_ARC_BG_START, _ARC_BG_START)
            self._prev_goal_met = goal_met
